...
async function assertTxFail(promise, msg) {
	try {
		...
		if (!txFailed) {
			assert.fail(
				'Unexpected error when checking for transaction failure',
				err.message
			);
		}
		...
		assert(
			msg,
			`Expected exception message is not defined. The actual exception was: "${err.message}". Stack: ${err.stack}`
		);
		assert(
			err.message.endsWith(msg),
			`Expected exception message "${msg}" was different from the actual exception message: "${err.message}". Stack: ${err.stack}`
		);
		return;
	}
